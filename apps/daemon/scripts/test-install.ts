...
function step4RunContainer(tgzPath: string): void {
  ...
  const containerScript = [
    ...
    `od-cli daemon start --no-open --port ${port} &`,
    `sleep ${DAEMON_START_WAIT_SEC}`,
    'echo "=== od-cli version ==="',
    `od-cli version --daemon-url http://127.0.0.1:${port}`,
    'echo "=== od-cli daemon status ==="',
    `od-cli daemon status --json --daemon-url http://127.0.0.1:${port}`,
    'echo "=== od-cli mcp install --print ==="',
    `od-cli mcp install --print codex --daemon-url http://127.0.0.1:${port}`,
    'echo "=== od-cli doctor ==="',
    `od-cli doctor --json --daemon-url http://127.0.0.1:${port} || true`,
