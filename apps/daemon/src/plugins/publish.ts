// Phase 4 / spec §14.1 — `od-cli plugin publish --to <catalog>` PR-template launcher.
//
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      const bodyWithRegistry = [
        ...
        '- Required checks: `od-cli plugin validate`, `od-cli plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('');
  lines.push('Generated by `od-cli plugin publish` — see https://open-design.ai/docs/plugins-spec.md.');
  return lines.join('\n');
}
