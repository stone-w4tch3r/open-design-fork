...
//
//   - `od-cli`            → SKILL.md + open-design.json (canonical OD shape).
...
// even after an `od-cli plugin update` rotates the live source.
...
export async function exportPlugin(input: ExportInput): Promise<ExportResult> {
  ...
  const readme = [
    ...
    '',
    'This folder was produced by `od-cli plugin export`.',
    '',
  ].join('\n');
