// Help surface for `od-cli brand`. Kept pure and separate from cli.ts so a test can
...
export const BRAND_USAGE = `Usage:
  od-cli brand list [--json]               List extracted brands (id, name, domain, status).
  od-cli brand extract <url> [--json]      Start an agent-driven brand extraction. Reserves the
  od-cli brand create <url> [--json]       brand and a backing project with the site open in a
                                        browser tab; open it to run the extraction agent.
                                        --prompt-file <path|-> reads the URL from a file or stdin.
  od-cli brand preview <id> [--json]       Re-render brand.html from the project's current
                                        brand.json so the kit page fills in live during
                                        extraction. --project <projectId> overrides the project.
  od-cli brand finalize <id> [--json]      Register the agent's extracted kit (brand.json in the
                                        backing project) as a design system; marks it ready.
                                        --project <projectId> overrides the backing project.
  od-cli brand get <id> [--json]           Print one brand's full detail (meta + brand + guide).
  od-cli brand delete <id> [--json]        Remove a brand and its registered design system.
