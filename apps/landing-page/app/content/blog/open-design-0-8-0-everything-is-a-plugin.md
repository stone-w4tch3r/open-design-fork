...
i18n:
  ...
  zh-tw:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>如果你是……</th><th>從這裡開始</th></tr></thead><tbody><tr><td>初次接觸 Open Design</td><td>下載桌面 app，讓它對著一個既有的設計系統引導出一個專案</td></tr><tr><td>已經在執行 Open Design</td><td>讓封裝版自動更新把你帶到 0.8.0；app 內的更新程式彈出視窗會帶你走過經驗證的安裝</td></tr><tr><td>正在打造一個外掛</td><td>用 <code>od-cli plugin scaffold --id &#x3C;name></code> 搭建鷹架，用 <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> 驗證，並透過跟市集裡每一個其他外掛相同的 OD 發布路徑開一個 PR</td></tr></tbody></table>
  ...
  ja:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>あなたが…</th><th>ここから始める</th></tr></thead><tbody><tr><td>Open Design が初めてなら</td><td>デスクトップアプリをダウンロードし、既存のデザインシステムに対してプロジェクトをブートストラップさせる</td></tr><tr><td>すでに Open Design を動かしているなら</td><td>パッケージ化された自動更新に 0.8.0 まで連れてきてもらう。アプリ内の updater のポップアップが、検証されたインストールを案内します</td></tr><tr><td>プラグインを作っているなら</td><td><code>od-cli plugin scaffold --id &#x3C;name></code> でスキャフォールドし、<code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> で検証し、マーケットプレイスの他のすべてのプラグインを出荷するのと同じ OD の publish 経路を通して PR を開く</td></tr></tbody></table>
  ...
  ko:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>당신이…</th><th>여기서 시작하라</th></tr></thead><tbody><tr><td>Open Design을 처음 쓴다면</td><td>데스크톱 앱을 내려받아 기존 디자인 시스템을 대상으로 프로젝트를 부트스트랩하게 하라</td></tr><tr><td>이미 Open Design을 돌리고 있다면</td><td>패키지 자동 업데이트가 당신을 0.8.0으로 데려가게 하라. 인앱 업데이터 팝업이 검증된 설치 과정을 안내한다</td></tr><tr><td>플러그인을 만들고 있다면</td><td><code>od-cli plugin scaffold --id &#x3C;name></code>으로 스캐폴드하고, <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code>으로 검증하고, 마켓플레이스의 다른 모든 플러그인을 출시하는 동일한 OD 퍼블리시 경로를 통해 PR을 열어라</td></tr></tbody></table>
  ...
  de:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>Wenn du …</th><th>Hier anfangen</th></tr></thead><tbody><tr><td>Neu bei Open Design</td><td>Lade die Desktop-App herunter und lass sie ein Projekt gegen ein bestehendes Designsystem bootstrappen</td></tr><tr><td>Schon Open Design einsetzt</td><td>Lass das paketierte Auto-Update dich auf 0.8.0 bringen; das In-App-Updater-Popup führt dich durch die validierte Installation</td></tr><tr><td>Ein Plugin baust</td><td>Scaffolde mit <code>od-cli plugin scaffold --id &#x3C;name></code>, validiere mit <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> und öffne einen PR über denselben OD-Publish-Pfad, der jedes andere Plugin im Marketplace ausliefert</td></tr></tbody></table>
  ...
  ru:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>Если вы…</th><th>Начните здесь</th></tr></thead><tbody><tr><td>Новичок в Open Design</td><td>Скачайте десктопное приложение и позвольте ему загрузить проект на основе существующей дизайн-системы</td></tr><tr><td>Уже используете Open Design</td><td>Позвольте упакованному автообновлению поднять вас до 0.8.0; всплывающее окно апдейтера в приложении проведёт вас через проверенную установку</td></tr><tr><td>Создаёте плагин</td><td>Создайте скелет с помощью <code>od-cli plugin scaffold --id &#x3C;name></code>, проверьте с помощью <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> и откройте PR через тот же путь публикации OD, который выпускает каждый другой плагин в маркетплейсе</td></tr></tbody></table>
  ...
  vi:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>Nếu bạn…</th><th>Bắt đầu ở đây</th></tr></thead><tbody><tr><td>Mới với Open Design</td><td>Tải ứng dụng desktop và để nó khởi tạo một dự án dựa trên một design system có sẵn</td></tr><tr><td>Đã đang chạy Open Design</td><td>Để tự động cập nhật đóng gói đưa bạn lên 0.8.0; popup cập nhật trong ứng dụng dẫn bạn qua bản cài được kiểm định</td></tr><tr><td>Đang xây một plugin</td><td>Scaffold bằng <code>od-cli plugin scaffold --id &#x3C;name></code>, kiểm định bằng <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code>, và mở một PR qua cùng đường xuất bản OD vốn ship mọi plugin khác trong marketplace</td></tr></tbody></table>
  ...
  ar:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>إن كنت…</th><th>ابدأ هنا</th></tr></thead><tbody><tr><td>جديدًا على Open Design</td><td>نزّل تطبيق سطح المكتب ودعه يُمهّد مشروعًا على نظام تصميم موجود</td></tr><tr><td>تُشغّل Open Design بالفعل</td><td>دع التحديث التلقائي المُغلَّف يأخذك إلى 0.8.0؛ نافذة المُحدِّث داخل التطبيق ترشدك عبر التثبيت المُتحقَّق منه</td></tr><tr><td>تبني إضافة</td><td>سقّل بـ <code>od-cli plugin scaffold --id &#x3C;name></code>، وتحقّق بـ <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code>، وافتح PR عبر مسار نشر OD نفسه الذي يشحن كل إضافة أخرى في السوق</td></tr></tbody></table>
  ...
  tr:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>Eğer…</th><th>Buradan başlayın</th></tr></thead><tbody><tr><td>Open Design'a yeniyseniz</td><td>Masaüstü uygulamasını indirin ve mevcut bir tasarım sistemine karşı bir projeyi önyüklemesine izin verin</td></tr><tr><td>Zaten Open Design çalıştırıyorsanız</td><td>Paketlenmiş otomatik güncellemenin sizi 0.8.0'a getirmesine izin verin; uygulama içi güncelleyici açılır penceresi sizi doğrulanmış kurulumda yönlendirir</td></tr><tr><td>Bir eklenti oluşturuyorsanız</td><td><code>od-cli plugin scaffold --id &#x3C;name></code> ile iskelet oluşturun, <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> ile doğrulayın ve marketplace'teki diğer her eklentiyi gönderen aynı OD yayımlama yolu üzerinden bir PR açın</td></tr></tbody></table>
  ...
  uk:
    ...
    bodyHtml: |
      ...
      <table><thead><tr><th>Якщо ви…</th><th>Почніть тут</th></tr></thead><tbody><tr><td>Новачок в Open Design</td><td>Завантажте десктопний застосунок і дозвольте йому завести проєкт проти наявної дизайн-системи</td></tr><tr><td>Уже запускаєте Open Design</td><td>Дозвольте пакетному автооновленню привести вас до 0.8.0; спливаюче вікно апдейтера всередині застосунку проведе вас через перевірене встановлення</td></tr><tr><td>Будуєте плагін</td><td>Скафолдіть за допомогою <code>od-cli plugin scaffold --id &#x3C;name></code>, перевірте за допомогою <code>od-cli plugin validate ./&#x3C;path> --no-daemon</code> і відкрийте PR через той самий шлях публікації OD, що постачає кожен інший плагін у маркетплейсі</td></tr></tbody></table>
...
| Building a plugin | Scaffold with `od-cli plugin scaffold --id <name>`, validate with `od-cli plugin validate ./<path> --no-daemon`, and open a PR through the same OD publish path that ships every other plugin in the marketplace |
