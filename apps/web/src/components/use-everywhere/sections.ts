...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    ...
    intro:
      ...
      "(`od-cli`) plus a Skills + Design-Systems + Atoms registry. Once it's " +
    ...
    bullets: [
      'CLI — `od-cli <command>` for headless scripts, CI, and shell automation.',
    ...
    snippets: [
      {
        ...
        language: 'bash',
        body: 'pnpm tools-dev\n# or, if `od-cli` is on your PATH (packaged install):\nod-cli --port 7456',
      },
      {
  ...
  },
  {
    ...
    intro:
      'The `od-cli` bin ships with the daemon and is the same binary used by ' +
    ...
    bullets: [
      '`od-cli` (no args) — boots the daemon and opens the web UI.',
      '`od-cli media generate ...` — produce image / video / audio bytes through the unified media protocol.',
      '`od-cli project create` + `od-cli run start` — create a project, send a message, and stream the run.',
      '`od-cli plugin install <source>` / `od-cli plugin apply <id>` — install and apply community plugins.',
      '`od-cli skills list` / `od-cli design-systems list` — inspect what is available locally.',
      '`od-cli status` / `od-cli doctor` — verify daemon health and detect agent CLIs on your PATH.',
    ],
    snippets: [
      {
      ...
      },
      {
        ...
        body:
          ...
          'PROJECT_JSON=$(od-cli project create \\\n' +
          '  --name "Investor pitch" \\\n' +
      ...
      },
      {
        ...
        language: 'bash',
        body:
          'od-cli files list "$PROJECT_ID" --daemon-url "$DAEMON_URL" --json\n' +
          'od-cli files read "$PROJECT_ID" index.html --daemon-url "$DAEMON_URL" | head',
      },
      {
        label: 'Inventory locally available skills and design systems',
        language: 'bash',
        body: 'od-cli skills list --json\nod-cli design-systems list --json',
      },
      {
        label: 'Check seeded artifacts through the CLI',
        language: 'bash',
        body:
          'od-cli project list --daemon-url http://127.0.0.1:7456\n' +
          'od-cli files list <seed-project-id> --daemon-url http://127.0.0.1:7456\n' +
          'od-cli files read <seed-project-id> index.html --daemon-url http://127.0.0.1:7456 | head',
      },
      {
    ...
    footer:
      ...
      'second instance for tests. From a source checkout, replace `od-cli` with ' +
  ...
  },
  {
    ...
    intro:
      'Open Design ships with a Model Context Protocol server (`od-cli mcp`) ' +
    ...
    bullets: [
      'Stdio transport — no extra port, the client spawns `od-cli mcp` directly.',
    ...
    snippets: [
      ...
      },
      {
        ...
        language: 'bash',
        body: 'od-cli mcp live-artifacts',
      },
    ],
    footer:
  ...
  },
  {
    ...
    bullets: [
      ...
      'Headless: an agent with `od-cli` on its PATH can call `od-cli skills list` then run any skill; the daemon is optional for read-only flows.',
    ...
    snippets: [
      ...
      },
      {
        ...
        language: 'bash',
        body: 'od-cli skills list --json | jq \'.skills[].name\'',
      },
      {
        ...
        body:
          ...
          'od-cli project list --json --daemon-url http://127.0.0.1:7456',
      },
    ],
    footer:
