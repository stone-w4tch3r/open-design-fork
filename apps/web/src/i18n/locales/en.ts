...
export const en: Dict = {
  ...
  'useEverywhere.section.overview.intro': 'Open Design is more than a window — it is a local privileged daemon (`od-cli`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od-cli <command>` for headless scripts, CI, and shell automation.',
  ...
  'useEverywhere.section.cli.intro': 'The `od-cli` bin ships with the daemon and is the same binary used by Claude Code / Codex when they run a generation. Most subcommands are thin clients that POST to the local daemon, so they work the same whether you launched it via `pnpm tools-dev` or as a packaged app.',
  'useEverywhere.section.cli.bullet1': '`od-cli` (no args) — boots the daemon and opens the web UI.',
  'useEverywhere.section.cli.bullet2': '`od-cli media generate ...` — produce image / video / audio bytes through the unified media protocol.',
  'useEverywhere.section.cli.bullet3': '`od-cli run ...` — start a project run from a prompt + skill.',
  'useEverywhere.section.cli.bullet4': '`od-cli plugin install <source>` / `od-cli plugin apply <id>` — install and apply community plugins.',
  'useEverywhere.section.cli.bullet5': '`od-cli skills list` / `od-cli design-systems list` — inspect what is available locally.',
  'useEverywhere.section.cli.bullet6': '`od-cli status` / `od-cli doctor` — verify daemon health and detect agent CLIs on your PATH.',
  ...
  'useEverywhere.section.mcp.intro': 'Open Design ships with a Model Context Protocol server (`od-cli mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover Open Design tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  'useEverywhere.section.mcp.bullet1': 'Stdio transport — no extra port, the client spawns `od-cli mcp` directly.',
  ...
  'useEverywhere.section.skills.bullet4': 'Headless: an agent with `od-cli` on its PATH can call `od-cli skills list` then run any skill; the daemon is optional for read-only flows.',
