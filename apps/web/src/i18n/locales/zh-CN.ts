...
export const zhCN: Dict = {
  ...
  'useEverywhere.section.overview.intro': 'Open Design 不只是一个窗口；它包含本地特权 daemon（`od-cli`），以及 Skills + Design Systems + Atoms registry。它在你的机器上运行后，任何代码智能体（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes 或你自己的脚本）都可以通过四种可互换入口驱动生成、检查项目并产出设计制品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 使用 `od-cli <command>` 支持无头脚本、CI 和 shell 自动化。',
  ...
  'useEverywhere.section.cli.intro': '`od-cli` 命令随 daemon 一起发布，也是 Claude Code / Codex 运行生成时使用的同一个二进制。多数子命令都是向本地 daemon 发送 POST 的轻量客户端，因此无论你通过 `pnpm tools-dev` 启动，还是通过打包应用启动，行为都一致。',
  'useEverywhere.section.cli.bullet1': '`od-cli`（无参数）— 启动 daemon 并打开 Web UI。',
  'useEverywhere.section.cli.bullet2': '`od-cli media generate ...` — 通过统一媒体协议生成图片 / 视频 / 音频字节。',
  'useEverywhere.section.cli.bullet3': '`od-cli run ...` — 从 prompt + skill 启动一次项目运行。',
  'useEverywhere.section.cli.bullet4': '`od-cli plugin install <source>` / `od-cli plugin apply <id>` — 安装并应用社区插件。',
  'useEverywhere.section.cli.bullet5': '`od-cli skills list` / `od-cli design-systems list` — 查看本地可用内容。',
  'useEverywhere.section.cli.bullet6': '`od-cli status` / `od-cli doctor` — 验证 daemon 健康状态并检测 PATH 中的智能体 CLI。',
  ...
  'useEverywhere.section.mcp.intro': 'Open Design 内置 Model Context Protocol 服务器（`od-cli mcp`），让任何支持 MCP 的客户端（Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes）都能发现 Open Design 工具（列出技能、渲染预览、生成媒体、运行插件），无需手动 shell 调用。daemon 会通过 `GET /api/mcp/install-info` 为主流客户端发布可直接粘贴的安装片段。',
  'useEverywhere.section.mcp.bullet1': 'Stdio 传输 — 无需额外端口，客户端直接启动 `od-cli mcp`。',
  ...
  'useEverywhere.section.skills.bullet4': '无头模式：PATH 中有 `od-cli` 的智能体可以调用 `od-cli skills list` 后运行任意 skill；只读流程不强制需要 daemon。',
