...
export const ru: Dict = {
  ...
  'useEverywhere.section.overview.intro': 'Open Design — это больше, чем окно: это локальный привилегированный демон (`od-cli`) плюс реестр Skills + Design-Systems + Atoms. Как только он запущен на вашей машине, любой код-агент (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, ваш собственный скрипт) может управлять генерациями, инспектировать проекты и создавать дизайн-артефакты через четыре взаимозаменяемые поверхности.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od-cli <command>` для headless-скриптов, CI и автоматизации в shell.',
  ...
  'useEverywhere.section.cli.intro': 'Исполняемый файл `od-cli` поставляется вместе с демоном и является тем же бинарником, который используют Claude Code / Codex при запуске генерации. Большинство подкоманд — это лёгкие клиенты, отправляющие POST-запросы локальному демону, поэтому они работают одинаково независимо от того, запустили ли вы его через `pnpm tools-dev` или как упакованное приложение.',
  'useEverywhere.section.cli.bullet1': '`od-cli` (без аргументов) — запускает демон и открывает веб-интерфейс.',
  'useEverywhere.section.cli.bullet2': '`od-cli media generate ...` — создаёт байты изображения / видео / аудио через единый медиа-протокол.',
  'useEverywhere.section.cli.bullet3': '`od-cli run ...` — запускает выполнение проекта на основе промпта + навыка.',
  'useEverywhere.section.cli.bullet4': '`od-cli plugin install <source>` / `od-cli plugin apply <id>` — устанавливает и применяет плагины сообщества.',
  'useEverywhere.section.cli.bullet5': '`od-cli skills list` / `od-cli design-systems list` — показывает, что доступно локально.',
  'useEverywhere.section.cli.bullet6': '`od-cli status` / `od-cli doctor` — проверяет состояние демона и обнаруживает CLI агентов в вашем PATH.',
  ...
  'useEverywhere.section.mcp.intro': 'Open Design поставляется с сервером Model Context Protocol (`od-cli mcp`), который позволяет любому клиенту с поддержкой MCP — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — обнаруживать инструменты Open Design (получать список навыков, отображать предпросмотры, генерировать медиа, запускать плагины) без ручного вызова через shell. Демон публикует готовый к вставке фрагмент установки через `GET /api/mcp/install-info` для каждого основного клиента.',
  'useEverywhere.section.mcp.bullet1': 'Транспорт stdio — без дополнительного порта, клиент запускает `od-cli mcp` напрямую.',
  ...
  'useEverywhere.section.skills.bullet4': 'Фоновый режим: Agent с `od-cli` в своём PATH может вызвать `od-cli skills list`, а затем запустить любой навык; для операций только на чтение демон необязателен.',
