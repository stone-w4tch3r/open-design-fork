...
```bash
od-cli marketplace add https://example.com/open-design-marketplace.json --trust restricted
od-cli marketplace refresh <id>
od-cli marketplace search "deck" --json
```
...
```bash
od-cli marketplace login https://github.example.com/org/plugin-registry
od-cli marketplace add https://raw.github.example.com/org/plugin-registry/main/open-design-marketplace.json --trust trusted
```
...
The CLI vocabulary stays the same: `od-cli marketplace add/search/doctor`,
`od-cli plugin install/upgrade/publish/yank`.
