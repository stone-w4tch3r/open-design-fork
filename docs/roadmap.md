...
5. A third party can publish a skill repo; `od-cli skill add <url>` installs it and it works.
...
- Topology B: Vercel web + tunneled local daemon
  - Ship a helper subcommand: `od-cli daemon --expose` using `cloudflared` (opt-in, documented)

**Dev experience:**
- `od-cli skill test` with cheap-model runs
- Skill author starter template: `od-cli skill scaffold`
...
| SE2 | Automation template registry exposed in both web UI and `od-cli automation`. |
...
`od-cli memory tree list/view/edit/move` expose a derived editable tree while the
...
`od-cli automation proposal list/get/apply/reject` can review memory-node, skill,
...
`/api/automation-source-packets`, and `od-cli automation source ingest/list/get`.
...
| Skill security (malicious skill via `od-cli skill add`) | user machine compromise | install-time warning; rely on agent's own permission model; document best practices |
