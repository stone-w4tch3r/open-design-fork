...
separated registry paths are used by catalogs, not by `od-cli plugin scaffold`.
...
`od-cli plugin pack` writes the archive next to the plugin folder by default.

## 3. Authenticate

```bash
od-cli plugin login
od-cli plugin whoami --json
```
...
```bash
od-cli plugin publish figma-workflow --to open-design --repo https://github.com/acme/figma-workflow
```
...
```bash
od-cli marketplace refresh official
od-cli plugin install figma-workflow
od-cli plugin info figma-workflow --json
```
...
```bash
od-cli plugin yank figma-workflow@1.0.0 --reason "Security issue"
```
