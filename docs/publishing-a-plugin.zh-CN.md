...
```bash
od-cli plugin scaffold --id vendor/plugin-name --title "Plugin name" --out ./plugins/community
```
...
```bash
od-cli plugin validate ./plugins/community/plugin-name
od-cli plugin pack ./plugins/community/plugin-name --out ./dist
```
...
```bash
od-cli plugin publish vendor/plugin-name --to open-design --repo https://github.com/vendor/plugin-name
```
...
od-cli plugin install vendor/plugin-name
od-cli plugin info vendor/plugin-name --json
```
...
```bash
od-cli plugin yank vendor/plugin-name@1.0.0 --reason "Security issue"
```
