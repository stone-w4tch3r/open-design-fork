...
> Open Design 以 **技能、CLI 和 MCP 服务器**的形式交付，主流编码 Agent 可原生消费。装好 OD 后，一行 `od-cli mcp install <agent>` 把 MCP 服务器 wire 进对应 Agent 的配置，任何 Agent 内调用相同工具。

| 编码 Agent / 平台 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | 状态 &nbsp;&nbsp; | 一行命令安装 MCP 服务器 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|---|:---:|---|
| [Claude Code](https://docs.anthropic.com/en/docs/claude-code) | ✅ 支持 | `od-cli mcp install claude` |
| [Codex CLI](https://github.com/openai/codex) | ✅ 支持 | `od-cli mcp install codex` |
| [Cursor](https://www.cursor.com/cli) | ✅ 支持 | `od-cli mcp install cursor` |
| [VS Code + GitHub Copilot](https://github.com/features/copilot) | ✅ 支持 | `od-cli mcp install copilot` |
| [GitHub Copilot CLI](https://github.com/features/copilot/cli) | ✅ 支持 | `od-cli mcp install copilot` |
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | ✅ 支持 | `od-cli mcp install gemini` |
| [OpenCode](https://opencode.ai/) | ✅ 支持 | `od-cli mcp install opencode` |
| [OpenClaw](https://github.com/openclaw/openclaw) | ✅ 支持 | `od-cli mcp install openclaw` |
| [Antigravity](https://antigravity.google) | ✅ 支持 | `od-cli mcp install antigravity` |
| [Cline](https://github.com/cline/cline) | ✅ 支持 | `od-cli mcp install cline` |
| [Trae](https://www.trae.ai/) | ✅ 支持 | `od-cli mcp install trae` |
| [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) | ✅ 支持 | `od-cli mcp install kimi` |
| [Pi Agent](https://github.com/badlogic/pi-mono) | ✅ 支持 | `od-cli mcp install pi` |
| [Mistral Vibe CLI](https://github.com/mistralai/mistral-vibe) | ✅ 支持 | `od-cli mcp install vibe` |
| [Hermes Agent](https://github.com/nousresearch/hermes-agent) | ✅ 支持 | `od-cli mcp install hermes` |

`od-cli mcp install <agent> --print` 干跑预览 · `--uninstall` 卸载 · 完整清单 `od-cli mcp install --help`。
...
# 然后，Agent 可以：
od-cli search-files "primary button"      # 跨项目文件搜索
od-cli get-file design-systems/linear-app/DESIGN.md
od-cli get-artifact <slug>                # 最新渲染的工件
...
插件在 **Web UI** 和 **`od-cli` CLI** 两条路上完全对等——同一套 `/api/plugins` 端点，挑顺手的用。
...
```bash
od-cli plugin list                       # 列出已安装插件（--task-kind / --mode / --tag 过滤）
od-cli plugin search "landing page"      # 按关键词搜
od-cli plugin info od-default            # 看某个插件的元数据、输入、权限
od-cli plugin install od-figma-migration # 从注册中心装；也支持 ./本地文件夹 或 https://… 直链
od-cli plugin apply od-default --input brief="给我们的种子轮做一页 pitch"
od-cli plugin upgrade od-default         # 升级
od-cli plugin uninstall od-default       # 卸载
```
...
```bash
od-cli plugin scaffold --id my-plugin --title "My Plugin"   # 生成骨架
od-cli plugin validate ./my-plugin                          # 校验 manifest / 文件布局
...
2. 跑通校验：`od-cli plugin validate`、`pnpm guard`、`pnpm --filter @open-design/plugin-runtime typecheck`。
...
- [ ] AI 输出的调参面板 UX——尚未实现
- [ ] `npx od-cli init` 脚手架创建带 `DESIGN.md` 的项目
- [ ] 插件 SDK + `od-cli plugin {add,list,remove,test,publish}` CLI
