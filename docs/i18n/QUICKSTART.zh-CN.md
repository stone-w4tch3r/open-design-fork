...
pnpm --filter @open-design/daemon build  # 构建 apps/daemon/dist/cli.js，供 `od-cli` 使用
...
Image、video、audio、HyperFrames 等 skill 在通过 daemon 启动 agent 时，会注入环境变量以调用本地 `od-cli` CLI：
...
└── package.json               # 根级质量脚本 + `od-cli` bin
```
