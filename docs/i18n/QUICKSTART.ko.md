...
pnpm --filter @open-design/daemon build  # `od-cli`용 apps/daemon/dist/cli.js 빌드
...
이미지, 비디오, 오디오, HyperFrames skill은 daemon이 에이전트를 spawn할 때 주입하는 환경 변수를 통해 로컬 `od-cli` CLI를 호출합니다.
...
└── package.json               # 루트 품질 스크립트 + `od-cli` bin
```
