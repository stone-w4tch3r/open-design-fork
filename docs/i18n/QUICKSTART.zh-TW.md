...
pnpm --filter @open-design/daemon build  # 建置 apps/daemon/dist/cli.js，供 `od-cli` 使用
...
Image、video、audio、HyperFrames 等 skill 在透過 daemon 啟動 agent 時，會注入環境變數以呼叫本地 `od-cli` CLI：
...
└── package.json               # 頂層品質腳本 + `od-cli` bin
```
