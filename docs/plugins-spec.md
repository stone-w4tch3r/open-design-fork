...
5. **UI is a collaboration surface.** The web/desktop UI can show forms, previews, direction pickers, critique panels, and live artifacts, but the same flow must work headlessly through `od-cli`.
...
| Headless CLI / code agent | Claude Code, Cursor, Codex, CI, or script shells out to `od-cli` | `od-cli run create --plugin make-a-deck --input audience=investors --input topic='AI design ops'` | Same manifest resolution, same context chips, same run events without opening desktop. |
...
| `od.capabilities_required` | `od.capabilities` | Map only capabilities that can be expressed; unknown capabilities are kept in `compatWarnings[]`, and `od-cli plugin doctor` must surface them |
...
Multiple marketplaces coexist — the user runs `od-cli marketplace add <url>` to register additional indexes (Vercel's, OpenClaw's clawhub, an enterprise team's private catalog). By default, a user-added marketplace is only a discovery source and plugins from it still install as `restricted`; only the built-in official marketplace or a marketplace explicitly trusted through `od-cli marketplace add <url> --trust` / `od-cli marketplace trust <id>` can pass through default `trusted` status.
...
Conflict resolution uses normalized `name` / plugin id; lower numeric priority wins. Legacy `SKILL.md` locations are synthesized into plugin records by the adapter, but are not copied into `<daemonDataDir>/plugins/` unless the user explicitly runs `od-cli plugin install`. This keeps existing Claude skills zero-config while giving plugin bundles a clear install root.

### 7.2 Install sources

```
od-cli plugin install ./folder
od-cli plugin install github:owner/repo
od-cli plugin install github:owner/repo@v1.2.0
od-cli plugin install github:owner/repo/path/to/subfolder
od-cli plugin install https://example.com/plugin.tar.gz
od-cli plugin install make-a-deck                   # via configured marketplaces
od-cli marketplace add https://.../open-design-marketplace.json
```
...
- A plugin can be upgraded between two runs via `od-cli plugin update`.
...
3. **Replay** — `od-cli run replay <runId>` and `od-cli plugin export <runId>` must reconstruct prompt and assets from the snapshot rather than the live manifest, so old runs remain reproducible after plugin upgrades.
...
Only the daemon writes `AppliedPluginSnapshot`; CLI/UI clients are read-only. Plugin upgrades or marketplace ref drift cause `od-cli plugin doctor` to mark affected historical snapshots as `stale`, but **never** to rewrite them: reproducibility wins over freshness.
...
A `restricted` plugin can never reach P3/P4/P5 unless the user grants the capability — either through `od-cli plugin trust <id>` or "Grant capabilities" on the detail page. Only two sources are trusted by default: repo-bundled first-party plugins and the official OD marketplace. User-added third-party marketplaces are discovery sources; plugins from them still install as `restricted` unless the marketplace itself is explicitly trusted, or an individual plugin is granted capabilities by id + version + capability.
...
1. **Pre-trust** (recommended for hosted / CI).
   ...
   ```bash
   od-cli plugin trust make-a-deck   --caps fs:read,mcp,subprocess
   od-cli plugin trust make-a-digest --caps fs:read,connector:slack,connector:notion
   od-cli plugin trust make-a-deck   --caps all          # equivalent to all capabilities the manifest declares
   ```
...
2. **Per-call temporary grant**.
   ...
   ```bash
   od-cli plugin apply make-a-deck   --project p_abc --grant-caps fs:read,mcp --json
   od-cli plugin apply make-a-digest --project p_abc --grant-caps fs:read,connector:slack --json
   od-cli plugin run   make-a-deck   --project p_abc --grant-caps fs:read --follow
   ```
   ...
   ```json
   {
     "error": {
       ...
       "data": {
         ...
         "remediation": [
           "od-cli plugin trust make-a-deck --caps mcp,subprocess",
...
- Preview iframes start with `sandbox="allow-scripts"` only. They do not get `allow-same-origin`, `allow-forms`, `allow-popups`, or `allow-downloads` by default. If a first-party preview needs an extra flag, it must declare that in the manifest and `od-cli plugin doctor` must mark it as an elevated preview.
...
| `research-search` | `od-cli research search` ([`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts)) | Tavily web research | new-generation |
| `media-image` / `media-video` / `media-audio` | `od-cli media generate` | Media generation with provider config | new-generation, tune-collab |
...
`(planned)` atoms are **not** implemented in v1, but their IDs are reserved here and in §5 schema to avoid a future name churn. `GET /api/atoms` returns only implemented atoms in v1; planned atoms emit a clear "not yet implemented" warning from `od-cli plugin doctor` rather than a generic "unknown atom" error.
...
- `until` is a lightweight expression evaluated by the daemon (only comparisons and known signal variables: `critique.score`, `iterations`, `user.confirmed`, `preview.ok`); it is **not** arbitrary JS. `od-cli plugin doctor` validates syntax.
...
4. The user can break out anytime via `od-cli run respond <runId> --json '{"action":"break-loop"}'` or the UI "Stop refining" button.

Two hard constraints on devloop:

- **`until` is required.** Pipelines with `repeat: true` but no `until` fail `od-cli plugin doctor` and the daemon refuses to execute them.
...
`od-cli plugin doctor` enforces at install / apply time that: (1) when `oauth.route === 'connector'`, `oauth.connectorId` is present in the plugin's own `od.connectors.required[]` or `od.connectors.optional[]`; (2) when `oauth.route === 'mcp'`, `oauth.mcpServerId` matches a name in the plugin's MCP server set.
...
5. **User revoke:** UI / CLI provide `od-cli ui revoke <surface-id>` to flip a row to `invalidated`. Common case: OAuth logout.
...
The ND-JSON stream from `od-cli run watch` / `od-cli run start --follow` includes `genui_surface_request` events. A third-party code agent has three response paths:

```bash
# Inspect pending surfaces on a run
od-cli ui list --run <runId> --json

# Read a single surface (kind / schema / prompt) for rendering or auto-fill
od-cli ui show <runId> <surface-id> --json

# Respond from any process; daemon writes to genui_surfaces, the run continues
od-cli ui respond <runId> <surface-id> --value-json '{"audience":"VC"}'
od-cli ui respond <runId> <surface-id> --skip          # triggers onTimeout='skip'
od-cli ui revoke  <projectId> <surface-id>             # cross-conversation revoke
```
...
```bash
od-cli ui prefill <projectId> --plugin <pluginId> --json '{"figma-oauth":"<token>","direction-pick":"editorial"}'
```
...
- `validate.ts` — JSON Schema (drives both runtime checks and `od-cli plugin doctor`).
...
| New `apps/daemon/src/plugins/snapshots.ts` | §8.2.1 immutable snapshot read/write; `status='stale'` flips driven by `od-cli plugin doctor`; provides the replay helper backing `POST /api/runs/:runId/replay`. |
...
CREATE TABLE applied_plugin_snapshots (
  ...
  status                  TEXT NOT NULL DEFAULT 'fresh', -- fresh | stale (set by `od-cli plugin doctor` after upgrade)
...
Migrations are additive only; existing `projects` / `runs` / `conversations` column semantics are untouched. The daemon writes per the schema at install / apply / run start / stage end. `od-cli plugin doctor` flips affected snapshots to `status='stale'` after a plugin upgrade by comparing `manifest_source_digest`, but **never** deletes or rewrites a snapshot row — historical reproducibility wins over storage cost.
...
| Method | Path | Purpose |
| --- | --- | --- |
| GET | `/api/applied-plugins/:snapshotId` | read an immutable snapshot; used for audit, replay, `od-cli plugin export` |
...
> **Transport equivalence (rule of thumb).** Every endpoint above is also exposed as a CLI subcommand and, where it fits MCP semantics, as an MCP tool. Code agents should use the CLI; only the desktop web app and `od-cli …` itself use HTTP directly. See §12 for the full CLI surface.
...
- Every `od-cli plugin export` / `od-cli files upload --to <target>` / `od-cli deploy ...` appends an `exportTargets` / `deployTargets` row but **never** mutates `sourcePluginSnapshotId`.
...
| ------------------- | ------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------ |
| **Headless**        | Daemon process only — no web bundle, no electron       | CI, servers, containers, Claude-Code-driven flows    | `od-cli daemon start --headless` (new flag, Phase 2) |
| **Web**             | Daemon + local web UI (no electron)                    | Browser-only setups, Linux without GUI dependencies  | `od-cli daemon start --serve-web` (new, Phase 2)     |
| **Desktop**         | Daemon + web bundle + electron shell                   | Full product experience (today's default)            | `od-cli` (current default, unchanged)                |
...
- **Marketplace browsing** — UI: grid + filters + previews. CLI: `od-cli plugin list/info`, `od-cli marketplace search`, `od-cli plugin info <id> --json` returns the same manifest the UI renders.
- **Plugin apply** — UI: click card, chips and inputs hydrate in place. CLI: `od-cli plugin apply <id> --json` returns the identical `ApplyResult`.
- **Run streaming** — UI: chat bubbles, todo list, progress chrome. CLI: `od-cli run start --follow` emits ND-JSON events from the same `PersistedAgentEvent` discriminated union the UI consumes.
- **Direction picker / question form** — UI: rendered as inline cards. CLI: emitted as structured events on stdout; agent or scripted wrapper picks an option by writing to stdin or via `od-cli run respond <runId> --json '{...}'`.
- **Live artifact preview** — UI: hot-reloading iframe. CLI: `od-cli files watch <projectId> --path <relpath>` streams change events; user opens the file with their own tool of choice.
...
- CI is a first-class citizen: a GitHub Action can `npm i -g @open-design/cli && od-cli daemon start --headless && od-cli plugin install … && od-cli run start --project … --follow`. No display, no electron, no per-step UI scripting.
- External products can embed OD by spawning a headless daemon and shelling out — `od-cli` is the public surface, internals are free to evolve.

The cost: a small handful of `od-cli daemon` flags and one new lifecycle subcommand (`od-cli daemon start/stop/status` with `--headless` / `--serve-web`). Implementation lands in Phase 2 alongside the CLI parity slice.
...
The CLI (`od-cli …`) is **the canonical agent-facing API** for Open Design. Plugin verbs are one slice of it; the rest of the CLI wraps the daemon's core capabilities — projects, conversations, runs, file operations, design library introspection, daemon control — so that any code agent can drive OD end-to-end through shell calls. This is the "natural-language project + task creation through CLI" path: a code agent reads a user's request, then issues a sequence of `od-cli …` calls instead of speaking HTTP.
...
| **CLI (`od-cli …`)**      | **Code agents shelling out, scripts, CI**             | [`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts)            |
| MCP stdio             | MCP-aware agents (Claude Code, Cursor, etc.)          | `od-cli mcp` and `od-cli mcp live-artifacts` (existing)                  |

When a new capability ships, the CLI subcommand is the primary contract. The HTTP route exists to back the CLI; the MCP server exposes a curated subset of CLI subcommands as tools. Versioning: subcommand names, argument names, and `--json` schemas are governed by `packages/contracts` and tested in CI; breaking changes follow a major-version bump of the `od-cli` bin.
...
od-cli project list   [--json]
od-cli project info   <id> [--json]
od-cli project delete <id>
od-cli project import <path> [--name "<title>"] [--json]   # wraps existing /api/import/folder
od-cli project open   <id>                                 # opens browser at /projects/<id>
```

Result of `od-cli project create --json`:
...
```
od-cli conversation list <projectId> [--json]
od-cli conversation new  <projectId> [--name "<title>"] [--json]
od-cli conversation info <conversationId> [--json]
```

#### Run / task lifecycle (new)

```
od-cli run start --project <projectId> [--conversation <conversationId>]
             [--message "<text>"] [--plugin <pluginId>] [--input k=v ...]
             [--agent claude|codex|gemini] [--model <id>] [--reasoning <level>]
             [--attachments <relpath,...>] [--follow] [--json]

od-cli run watch  <runId>                # ND-JSON SSE-equivalent events on stdout
od-cli run cancel <runId>
od-cli run list   [--project <id>] [--status running|done|failed] [--json]
od-cli run logs   <runId>                # historical tail; --since for incremental
```
...
```
od-cli files list   <projectId> [--path <subdir>] [--json]
od-cli files read   <projectId> <relpath>                   # writes to stdout
od-cli files write  <projectId> <relpath> [< stdin]         # reads from stdin
od-cli files upload <projectId> <localpath> [--as <relpath>]
od-cli files delete <projectId> <relpath>
od-cli files diff   <projectId> <relpath>                   # vs. last committed version (when imported from git)
```

A code agent typically uses `od-cli files read` / `od-cli files write` instead of native file ops when targeting OD-managed projects, because the daemon owns artifact bookkeeping (`ArtifactManifest.sourceSkillId`, etc. in [`packages/contracts/src/api/registry.ts`](../packages/contracts/src/api/registry.ts)).

#### Plugin verbs

```
od-cli plugin install   <source>                            # github: | https://… | ./folder | <name from marketplace>
od-cli plugin uninstall <id>
od-cli plugin list      [--kind skill|scenario|atom|bundle] [--trust trusted|restricted] [--json]
od-cli plugin info      <id> [--json]
od-cli plugin update    [<id>]
od-cli plugin trust     <id> [--caps fs:write,mcp,bash,hooks]   # permanent grant; persisted on installed_plugins
od-cli plugin apply     <id> --project <projectId> [--input k=v ...] [--grant-caps fs:read,mcp ...] [--json]
                                                            # returns ApplyResult; pure (no run)
                                                            # --grant-caps: scoped to this apply's snapshot only
od-cli plugin run       <id> --project <projectId> [--input k=v ...] [--grant-caps ...] [--follow] [--json]
                                                            # shorthand: apply + run start --follow
od-cli plugin replay    <runId> [--follow] [--json]             # rerun the long-task using the run's applied_plugin_snapshot
od-cli plugin export    <projectId> --as od|claude-plugin|agent-skill --out <dir>
od-cli plugin doctor    <id>
od-cli plugin scaffold
```

#### Generative UI verbs (§10.3)

```
od-cli ui list      [--run <runId> | --project <projectId>] [--status pending|resolved|timeout|invalidated] [--json]
od-cli ui show      <runId> <surface-id> [--json]                 # surface kind / schema / prompt
od-cli ui respond   <runId> <surface-id> --value-json '{...}'     # write answer, unblock the run
od-cli ui respond   <runId> <surface-id> --skip                   # trigger onTimeout='skip'
od-cli ui revoke    <projectId> <surface-id>                      # invalidate persisted answer (e.g. OAuth logout)
od-cli ui prefill   <projectId> --plugin <pluginId> --json '{ "<surface-id>": <value>, ... }'
...
```
od-cli marketplace add     <url> [--trust]
od-cli marketplace remove  <id>
od-cli marketplace trust   <id>
od-cli marketplace untrust <id>
od-cli marketplace list    [--json]
od-cli marketplace refresh [<id>]
od-cli marketplace search  "<query>" [--tag <tag>] [--json]   # search across configured catalogs
```

#### Design library introspection (new)

```
od-cli skills list             [--json] [--scenario <s>] [--mode <m>]
od-cli skills show             <id> [--json]
od-cli design-systems list     [--json]
od-cli design-systems show     <id> [--json]
od-cli craft list              [--json]
od-cli atoms list              [--json]                       # first-party atoms (§10)
```

#### Daemon control (new)

```
od-cli daemon start  [--headless] [--serve-web] [--port <n>] [--host <h>]
                                                           # explicit lifecycle (§11.7);
                                                           # default `od-cli` (no args) keeps current behavior
od-cli daemon stop   [--daemon-url <url>]
od-cli daemon status [--json]                                   # alias of `od-cli status`
od-cli status        [--json]                                   # daemon up? port? installed plugins count
od doctor                                                   # diagnostics: skills/DS/craft/plugins, providers, MCP
od version       [--json]
od-cli config get|set|list|unset  [--key ...] [--value ...]     # backed by media-config.json + db
```

`od-cli daemon start --headless` is the entry for the headless mode in §11.7 (no web bundle, no electron). `od-cli daemon start --serve-web` adds the local web UI without electron. Both keep using the existing tools-dev port conventions ([`OD_PORT`, `OD_WEB_PORT`](../AGENTS.md)).

#### Existing agent-callable tools (unchanged)

```
od-cli research search ...
od-cli media generate  ...
od-cli tools live-artifacts ...
od-cli tools connectors  ...
od-cli mcp                       # stdio MCP server
od-cli mcp live-artifacts        # specialized MCP server
```
...
| --- | --- | --- | --- |
| 64 | Daemon not running | `od-cli status`, then start daemon | `{ host, port }` |
| 65 | Plugin not found / not installed | `od-cli plugin list` then `od-cli plugin install <source>` | `{ pluginId, candidateSources[] }` |
| 66 | Plugin restricted, capability required | `od-cli plugin trust <id> --caps …` or retry with `--grant-caps …` | `{ pluginId, pluginVersion, required[], granted[], remediation[] }` |
| 67 | Required input missing on apply | re-run with `--input k=v` for each missing field | `{ pluginId, missing[], schema }` |
| 68 | Project not found | `od-cli project list` | `{ projectId }` |
| 69 | Run not found / already terminal | `od-cli run list --project <id>` | `{ runId, status }` |
| 70 | Provider not configured | `od-cli config set ...` for the provider key | `{ provider, requiredKeys[] }` |
| 71 | Plugin requires daemon mode | start daemon or switch to desktop / headless | `{ pluginId, mode: 'api-fallback' }` (§11.8) |
| 72 | Applied plugin snapshot stale | `od-cli plugin replay <runId>` or `od-cli plugin upgrade <id>` then re-apply | `{ snapshotId, pluginId, currentVersion, snapshotVersion }` |
| 73 | GenUI surface awaiting response | inspect via `od-cli ui show <runId> <surface-id>` then answer with `od-cli ui respond`, or `od-cli ui prefill` before the run | `{ runId, surfaceId, kind, schema, prompt, persist, timeoutAt }` (§10.3) |
...
od-cli skills list --json
od-cli plugin list --json

# 2. Create a project bound to a skill or design system.
PID=$(od-cli project create --skill blog-post --design-system linear-clone --json | jq -r .projectId)

# 3. Apply a plugin to preview the brief and context (pure; no run yet).
od-cli plugin apply make-a-deck --project "$PID" --input topic="agentic design" --input audience=VC --json

# 4. Start the run, follow events live (ND-JSON on stdout).
od-cli run start --project "$PID" --plugin make-a-deck \
...
od-cli files list "$PID" --json
od-cli files read "$PID" index.html > out.html
```
...
Every group above is additive to [`apps/daemon/src/cli.ts`](../apps/daemon/src/cli.ts). The current default `od-cli` (start daemon + open web UI) remains unchanged. Existing `od-cli media`, `od-cli research`, `od-cli tools`, `od-cli mcp` commands keep their exact contracts. The new groups are wrappers around endpoints that already exist in `apps/daemon/src/server.ts` for the ones the desktop UI uses today (project create/list, run start/watch, file upload/list), plus the new endpoints from §11.5 for plugins/marketplace/atoms.

> **Implementation rule:** if a code agent can do something through the desktop UI, it MUST be doable through `od-cli …` with the same arguments and equivalent output. No silent UI-only capabilities.
...
- `od-cli plugin scaffold` — writes a starter folder containing both `SKILL.md` (industry-standard, with `od:` frontmatter for backward compat) and `open-design.json` (OD enrichment with `compat.agentSkills` pointing at the SKILL.md).
- `od-cli plugin doctor` — runs the JSON Schema, the SKILL.md frontmatter parser, and a "does this look listable on awesome-agent-skills / clawhub / skills.sh?" lint that checks for README presence, license file, and frontmatter completeness.
- `od-cli plugin publish --to <catalog>` (Phase 4) — opens a browser to the catalog's PR template with a pre-filled row.
...
### 14.3 Concrete headless pipeline (Claude Code + `od-cli` CLI, no OD UI)
...
# Path A — let `od-cli` orchestrate Claude Code as the run's agent:
od-cli run start --project "$PID" --plugin make-a-deck \
             --agent claude --follow
...
CWD=$(od-cli project info "$PID" --json | jq -r .cwd)
cd "$CWD"
...
od-cli files list "$PID" --json
od-cli files read "$PID" slides.html > slides.html
...
### 14.4 Analogy: Cursor vs `cursor-agent`, OD desktop vs `od-cli` CLI

The mental model:

| Layer                 | Cursor                                       | Open Design                                          |
| --------------------- | -------------------------------------------- | ---------------------------------------------------- |
| Headless agent CLI    | `cursor-agent` (drives the agent loop)       | `od-cli run start --agent claude --follow` + `od-cli plugin run` |
...
ANTHROPIC_API_KEY=...                # provider keys; also storable via `od-cli config set`
TAVILY_API_KEY=...
```
...
Anything settable via the desktop UI is also settable via `docker exec od od-cli config set ...`. This document MUST NOT provide concrete storage paths.
...
```bash
docker exec od od-cli plugin install github:open-design/plugins/make-a-deck
docker exec od od-cli project create --plugin make-a-deck --json
docker exec od od-cli status --json
```
...
- `OD_TRUST_DEFAULT=restricted` is the recommended default. Capabilities (`mcp`, `subprocess`, `bash`, `network`) require explicit operator opt-in via `od-cli plugin trust <id>` or a `OD_TRUSTED_PLUGINS` allow-list env var.
...
1. **Self-hosted enterprise.** A company hosts a private OD instance, registers an internal `open-design-marketplace.json` (`od-cli marketplace add https://internal/...`), restricting plugins to internally vetted ones. Their designers and PMs use the desktop client locally; their CI uses `docker exec od od …`.
...
- **Plugin CLI verbs:** `od-cli plugin install/list/info/uninstall/apply/doctor`. `od-cli plugin apply --json` is required by Phase 2's inline rail and by external code agents, and must already return an `ApplyResult` containing `appliedPlugin: AppliedPluginSnapshot`.
- **Headless MVP CLI loop (newly pulled forward):** `od-cli project create/list/info`, `od-cli run start/watch/cancel` (with `--follow` and ND-JSON streaming), `od-cli files list/read`. These wrap endpoints already used by the desktop UI today (`POST /api/projects`, `POST /api/runs`, `GET /api/runs/:id/events`, project list/read endpoints) — no new HTTP surface, only CLI surface.
...
- **End-to-end headless smoke** (equivalent to the §12.5 walkthrough): `od-cli plugin install ./fixtures/sample-plugin` → `od-cli project create --plugin <id> --json` → `od-cli run start --project <pid> --plugin <id> --follow` → `od-cli files read <pid> <artifact>`. The produced artifact bytes must match exactly what the same plugin produces under the Phase 2A UI flow.
- **Apply purity smoke:** after `od-cli plugin apply <id>` followed by cancel-before-send, the project cwd is empty of staged assets, no `.mcp.json` is generated, but the `applied_plugin_snapshots` row exists (unreferenced from any run/project).
...
- **§10.3 GenUI minimum surface set lands:**
  ...
  - CLI: `od-cli ui list/show/respond/revoke/prefill`; `od-cli run watch` ND-JSON includes `genui_*` events; exit code 73 wired in.
...
- **§9 connector capability gate lands:**
  ...
  - `od-cli plugin doctor` enforces: (a) every `od.connectors.required[].id` exists in `connectorService` catalog; (b) every `tools[]` is a subset of that connector's `allowedToolNames`; (c) when an `oauth-prompt` surface has `oauth.route='connector'`, its `oauth.connectorId` is one of the plugin's declared connectors.
  - Exit code 66 / `409 capabilities-required` `data.required` includes `connector:<id>` entries.
  - `od-cli plugin trust` and `od-cli plugin apply --grant-caps` accept the `connector:<id>` form (§9.1).
...
(c) `od-cli plugin replay <runId>` reproduces the **exact same prompt** even after the source plugin has been upgraded via `od-cli plugin update` (snapshot is immutable).
...
(e) A plugin declares both `oauth-prompt` and `confirmation` surfaces: after conversation A completes them, conversation B (same project) re-applies the plugin. In the new run the `oauth-prompt` (`persist=project`) is served from cache; the `confirmation` (`persist=run`) re-asks. After `od-cli ui revoke`, the next run re-asks the `oauth-prompt`.
...
(f) **Connector trust gate.** A local plugin declares `od.connectors.required = [{ id: 'slack', tools: ['channels.list'] }]` but **does not** explicitly grant `connector:slack`:
  ...
  - `apply` (no `--grant-caps`) → exit 66 / 409 with `data.required` including `connector:slack` and `data.remediation` listing `od-cli plugin trust <id> --caps connector:slack`.
...
- `od-cli project delete/import`, `od-cli run list/logs --since`, `od-cli files write/upload/delete/diff`, `od-cli conversation list/new/info`.
- These wrap existing endpoints; no new HTTP surface.

Validation: extend the §12.5 walkthrough — `od-cli project import` an external folder → `od-cli plugin apply` → `od-cli run replay <runId>` reruns on top of the imported project.

### Phase 3 — Federated marketplace + tiered trust (3–5 days)

- `od-cli marketplace add/remove/trust/untrust/list/refresh`; `od-cli plugin install <name>` resolves through marketplaces.
...
- `od-cli plugin doctor <id>` runs full validation.
...
- `od-cli plugin export <projectId> --as od|claude-plugin|agent-skill` — generates a publish-ready folder from an existing project.
- `od-cli plugin run <id> --input k=v --follow` — shorthand wrapper for apply + run start + watch.
- `od-cli plugin scaffold` interactive starter.
- `od-cli plugin publish --to anthropics-skills|awesome-agent-skills|clawhub` opens a PR template.
- **Remaining CLI parity:** `od-cli conversation list/new/info`, `od-cli skills/design-systems/craft/atoms list/show`, `od-cli status/doctor/version`, `od-cli config get/set/list`, `od-cli marketplace search`. All purely CLI work — endpoints exist or are trivial.
...
Validation: (a) install a published plugin → export from a real project that used it → diff the produced manifest against the original. (b) "UI vs CLI parity test": pick 5 desktop-UI workflows, replay each one through `od-cli …` only, compare produced artifacts byte-for-byte (per the §12.6 implementation rule).
...
- `docker run` smoke: image starts, web UI renders, `od-cli plugin install` works inside the container.
...
| Project-local plugins committed to user repos | Discovery only at `<projectCwd>/.open-design/plugins/`; opt-in via `od-cli plugin install --project`. |
...
| Plugins shipping their own MCP servers may fail to start | `od-cli plugin doctor` runs a dry-launch of declared MCP commands; failures surfaced before "Use". |
| Unbounded growth of `applied_plugin_snapshots` | Per PB2 (resolved): unreferenced snapshots auto-expire at `applied_at + OD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default 30 d); referenced snapshots stay pinned forever (reproducibility wins); GC worker lands in Phase 5 (§16). `od-cli plugin snapshots prune --before <ts>` remains as a forced-cleanup escape hatch; rows with `status='stale'` can be archived to external storage in batch. |
...
| Cross-conversation reuse via `genui_surfaces` may make users "forget what they authorized" | The web `GenUIInbox` and `od-cli ui list --project <id>` must enumerate every `persist=project` resolved row with revoke entry points; hosted mode can default-expire via `OD_GENUI_PROJECT_TTL_DAYS`; revoke writes an audit log entry. |
...
- **`od-cli plugin run` headless contract** — sufficient as-is, or also expose an HTTP POST endpoint for non-CLI agents? (Default: CLI only in v1; HTTP added in Phase 4 if needed.)
...
- **Discovery-time hot reload** — should the daemon watch `<daemonDataDir>/plugins/` for filesystem changes (developer ergonomics), or only reload after `od-cli plugin install/update/uninstall` (stability)? (Default: watch, with a 500ms debounce.)
- **Versioning policy** — pin to a tag/SHA on install, or always track the default branch with an opt-in pin? (Default: pin to the resolved ref at install time; `od-cli plugin update` re-resolves.)
...
- ~~**Should `od.genui.surfaces[].component` ship in v1?**~~ — **resolved as a gated extension path.** The manifest schema accepts the field and `od-cli plugin doctor` enforces `genui:custom-component` plus traversal guards. The built-in product renderer remains the default for `form` / `choice` / `confirmation` / `oauth-prompt`; custom components are sandboxed add-ons, not a replacement for core collaboration UI.
- **Coupling between GenUI persisted state and `AppliedPluginSnapshot`** — when a plugin upgrades and `surface.schema` changes, old rows auto-`invalidate`; should we additionally **force a re-apply** (generating a new `AppliedPluginSnapshot`) or allow the surface to invalidate while leaving the snapshot untouched? (Default: surface only; `od-cli plugin doctor` flags schema drift; replay still uses the old snapshot.)
...
- **CLI from day 1.** Every new endpoint has a matching `od-cli plugin …` subcommand, so the same surface is reachable from any code agent without the desktop app.
...
- Live preview through `live-artifact` and the `od-cli files watch` CLI primitive (§12) means hot reloading and CLI co-watching both work in v1.
...
- "Design → production code" in v1 is a **two-product handoff**: OD owns the design substrate (SKILL.md / DESIGN.md / craft / generated artifacts staged into project cwd, plus `od-cli files`-managed artifact bookkeeping); the user's existing code agent (Cursor / Claude Code / Codex / Gemini CLI) owns the actual repo patch in the user's repo cwd.
- The handoff surface is the §14.3 headless pipeline plus `od-cli files read` / `od-cli files watch` for the code agent to consume artifacts inline.
...
1. **OD stages the design substrate into a project cwd.** Per §14.3, the daemon writes SKILL.md / DESIGN.md / craft into a staged skill-context directory and generated artifacts into the project cwd via `od-cli files`. The cwd is discoverable via `od-cli project info <id> --json | jq -r .cwd`.
...
3. **Bookkeeping stays in OD.** `ArtifactManifest` (§11.5.1) records `sourcePluginSnapshotId`, `sourceTaskKind: 'tune-collab'` (or `'code-migration'` once Phase 7 lands), and `handoffKind: 'patch'`; `od-cli files` tracks every artifact byte. Even when the code agent does the patch, OD remains the audit log.
4. **Re-entry into OD is single-step.** The user can reapply any plugin (or a different plugin) on top of the same project at any time via the inline rail (§8) or `od-cli plugin apply ... --project <id>`. `parentArtifactId` chaining (§11.5.1) preserves the lineage across the OD ↔ code-agent boundary.
...
- **Substrate** = the primitives the v1 spec hands to plugin authors: manifest fields, capability vocabulary, atom catalog, pipeline / devloop / GenUI / connector / MCP / `od-cli files` / `parentArtifactId` / `AppliedPluginSnapshot` (§5–§11.5.1).
...
| Operate on the user's real repo | `od-cli project import <path>` brings the repo into OD's project model; `od-cli files` and agent file ops then work in-place | §12 / §11.7 / §14.3 |
...
| Reproducible replay months later | `AppliedPluginSnapshot` immutable + `od-cli plugin replay` | §8.2.1 / §12 |
...
Pre-step the user runs: `od-cli project import /path/to/old-repo`. Project cwd is now the real repo.
...
- `od-cli files` retains audit; `parentArtifactId` chains the patch artifact to the design artifact.
...
| `trusted` | First-party / explicitly-trusted marketplace | No — auto-granted on install | Yes — `od-cli plugin update` may pull a newer version | `marketplace` |
...
3. `od-cli plugin uninstall` refuses to uninstall a `bundled` plugin (would break the daemon); `od-cli plugin update` is a no-op for bundled.
